...
def test_circle_detection_ui_flow(e2e_base_url, browser_page):
    ...
    expect(page.locator("#muPlotImage")).to_be_visible()
    mu_src = page.locator("#muPlotImage").get_attribute("src")
    assert mu_src is not None and mu_src.startswith("data:image/png;base64,")
    assert len(mu_src) > len("data:image/png;base64,")

    expect(page.locator("#diagonalSummary")).to_be_visible()
    summary_text = (page.locator("#diagonalSummary").text_content() or "").strip()
    assert summary_text != ""
